...
▪ FastQC reports<br>
▪ Per-sample Snippy variant calls<br>
▪ core.full.aln<br>
▪ core.aln<br>
▪ core.vcf<br>
▪ Optional Gubbins recombination-filtered alignment<br>
▪ IQ-TREE2 Newick tree<br>
▪ Tree visualization as PNG or SVG<br>
...
"SNPPhylogenomics.reference_type": "genbank"
```
For phylogeny-only workflows, Genbank is often more stable & sufficient.
...
## Example phylogenetic tree output

The workflow generates a rendered phylogenetic tree image from the final IQ-TREE2 Newick tree. Branch labels represent ultrafast bootstrap support values (%) from 1,000 replicates.

<p align="center">
  <img src="docs/images/phylogenetic_tree.png" alt="Example SNPPhylogenomics phylogenetic tree" width="850"/>
</p>

### Phylogenetic tree

Example maximum-likelihood phylogenetic tree generated by SNPPhylogenomics. The tree was inferred from a core-genome SNP alignment using IQ-TREE2 & visualized with ETE3. Branch labels indicate ultrafast bootstrap support values.

## Interactive phylogenetic visualization with Interactive Tree of Life (iTOL)

The workflow generates Newick tree files that can be used for advanced, interactive visualization & interpretation. Integrating SNP-based phylogenies with metadata in iTOL enables intuitive exploration of genomic epidemiology patterns, supporting outbreak investigation & AMR surveillance 

### Adding metadata (recommended)

To enhance interpretation, upload metadata alongside the tree.
...
▪ Country / location  
▪ Year of collection  
▪ Source or type (clinical, environmental)  
▪ AMR genes (e.g., *blaCTX-M*, *blaNDM*)  
▪ Sequence type (MLST)

Metadata should be formatted as a tab-delimited file compatible with iTOL annotation formats.

Example insights enabled

▪ Identification of transmission clusters (e.g., ≤5 SNPs)
▪ Visualization of lineage structure
▪ Detection of outbreak signals
▪ Integration of AMR profiles with phylogeny

### Locate tree outputs

```bash
find cromwell-executions -name "*.treefile"
find cromwell-executions -name "*.nwk"
```
### Use cleaned tree for best results in iTOL
...
| File                              | Description                                           |
|-----------------------------------|-------------------------------------------------------|
| final.treefile                    | Maximum-likelihood phylogeny from IQ-TREE2            |
| phylogenetic_tree.nwk     | Cleaned Newick tree optimized for downstream tools    |
...
▪ final.treefile or
▪ phylogenetic_tree.nwk (recommended)
▪ Visualize the phylogeny interactively
...
| 80–94 | Strong support |
| 70–79 | Moderate support |
| <70 | Weak or uncertain support |
